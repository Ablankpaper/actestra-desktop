...
process or temporary directory remains. The local, unpushed correction makes
supervisor stdin EOF terminal for the sidecar and its descendants, verifies the
whole POSIX process group rather than only the leader during explicit teardown,
and gives failed focused tests their own TERM-to-KILL cleanup fallback. Both new
parent-death and surviving-descendant tests failed before the correction and
passed after it; the complete sidecar-process file passes 10 tests, four-path
lint reports zero warnings, strict TypeScript passes, and the downstream
contract remains 237 declared paths, 4 R0 invariants, and 91 source copies.
No root gate, push, PR, or Actions run was performed for this correction. Real
CrewAI admission and P6 phase acceptance remain open.
...
| P6 PR 46 Team implementation and no-orphan gate    | Merged implementation; P6 acceptance reopened                     | Exact head `eaae304ef6e4a3d555fd03885f316de62c07a83b`; exact-head CI 30946794552; squash merge `2b6821bd7b65f6debcbdaf5a40a0daaea6909885`; merged-main CI 30948418563; post-merge live PIDs 91520/91521 disproved parent-death no-orphan coverage; local unpushed correction has parent-death and surviving-descendant RED/GREEN, 10/10 focused tests, zero-warning lint, strict TypeScript, 237/4/91 downstream evidence, and an empty matching-process/temp scan; no corrective root gate, push, PR, Actions, CrewAI admission, or P6 acceptance |
...
8. Preserve P5.3 exact head `15a8e29b44a6684313f29a694f0d0615de95cf36`,
   ...
   preservation.
9. Treat PR 46 as merged implementation evidence, not P6 acceptance. Keep the
   exit gate open until the planner parent-death and whole-process-group
   correction receives its separately authorized remote closure, then continue
   real CrewAI admission only after exact pin/rollback, locked dependencies,
...
- The P3 contracts and fake-worker gate have passed. The bounded deterministic
  ...
  focused native and DOM tests. The local correction adds focused parent-death
  and whole-group teardown evidence, but has no root, remote, or merged-main
